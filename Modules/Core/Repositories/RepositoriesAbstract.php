...
abstract class RepositoriesAbstract implements RepositoryInterface
{
    ...
    {
        if(isset($data['uuid'])){
            $model = $this->model->where('uuid',$data['uuid'])->first();
        }else{
    ...
    public function byPageItems($page = 1, $limit = 10,$items)
    {
        $result = new stdClass;
        $result->page = $page;
        $result->limit = $limit;
    ...
    public function adjacent($direction, $model,array $with = [], $all = false)
    {
        $currentModel = $model;
    ...
    public function allForApi(array $with = array())
    {
        $query = $this->make($with);
